...
def load_align_model(language_code, device, model_name=None, model_dir=None):
    if model_name is None:
        # use default model
        if language_code in DEFAULT_ALIGN_MODELS_TORCH:  # 如果是torch默认的模型就用torch的
            model_name = DEFAULT_ALIGN_MODELS_TORCH[language_code]
        elif language_code in DEFAULT_ALIGN_MODELS_HF:  # 一种是hf提供的.
    ...
    if model_name in torchaudio.pipelines.__all__:  # 一些官方的模型.
...
def align(
    ...
    for sdx, segment in enumerate(transcript):
        ...
        f1 = int(t1 * SAMPLE_RATE)  # 计算这个句子的开始针,结束帧
        ...
        with torch.inference_mode():
            if model_type == "torchaudio":
                emissions, _ = model(
                    waveform_segment.to(device), lengths=lengths)
